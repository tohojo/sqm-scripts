...
[ -z "$SHAPER" ] && SHAPER=htb && [ "$QDISC" = "cake" ] && SHAPER=cake
[ -z "$LLAM" ] && LLAM="default"
...
[ -z "$DIFFSERV_INGRESS" ] && DIFFSERV_INGRESS="diffserv3"
[ -z "$DIFFSERV_EGRESS" ] && DIFFSERV_EGRESS="diffserv3"
[ -z "$SCRIPT" ] && SCRIPT=""
...
[ -z "$MODINFO" ] && MODINFO=$(which modinfo)
[ -z "$TARGET" ] && TARGET="5ms"
...
ALL_SQM_VARS="IFACE UPLINK DOWNLINK SCRIPT ENABLED QDISC SHAPER QDISC_PRESET LLAM LINKLAYER OVERHEAD STAB_MTU STAB_MPU STAB_TSIZE AUTOFLOW ILIMIT ELIMIT TARGET ITARGET ETARGET IECN EECN ZERO_DSCP_INGRESS IGNORE_DSCP_INGRESS ZERO_DSCP_EGRESS IGNORE_DSCP_EGRESS DIFFSERV_INGRESS DIFFSERV_EGRESS IQDISC_OPTS EQDISC_OPTS SHAPER_BURST HTB_QUANTUM_FUNCTION SQM_VERBOSITY_MAX SQM_DEBUG SQM_DEBUG_LOG OUTPUT_TARGET"

# These are the modules that do_modules() will attempt to load
ALL_MODULES="act_ipt sch_$QDISC sch_ingress act_mirred cls_fw cls_flow cls_u32 sch_$SHAPER"
